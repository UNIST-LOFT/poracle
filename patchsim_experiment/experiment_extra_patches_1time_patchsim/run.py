...
def checkout(project, bugid, patch_no):
    logger.info('Check out {} {}'.format(project, bugid))
    os.system('defects4j checkout -p ' + project + ' -v ' + bugid + 'b' +
              ' -w ' + project + bugid + 'b')
    if os.path.exists('./' + project + bugid + 'b_' + patch_no):
        return
    os.system('cp -r ' + project + bugid + 'b' + ' ' + project + bugid +
              'b_bak')
    os.system('patch -u -p0 < ../patches/' + patch_no)
    os.system('mv ' + project + bugid + 'b ' + project + bugid + 'b_' +
              patch_no)
    os.system('mv ' + project + bugid + 'b_bak ' + project + bugid + 'b')


def gen_test_randoop(project, bug_id):
    logger.info('Generate tests with Randoop')
    if not os.path.exists('../test_gen_randoop/' + project + '/randoop/' +
                          str(bug_id)):
        os.system('run_randoop.pl -p ' + project + ' -v ' + str(bug_id) +
                  'b -n ' + str(bug_id) + ' -o ../test_gen_randoop -b 1800')
...
def trace(project, bugid, patch_no):
    ...
    if not os.path.exists('../randoop_cover/' + project + bugid + 'b_' +
                          patch_no + ".txt"):
    ...
    if not os.path.exists('../randoop_cover/' + project + bugid + 'b_' +
        ...
        randoop_tests = pylib.coverage.process_cover_trace(
            '../randoop_cover/' + project + bugid + 'b_' + patch_no + ".txt",
            20)

    if not os.path.exists('../test_coverage/' + project + bugid + 'b_' +
                          patch_no + ".txt"):
    ...
    if not os.path.exists('../test_coverage/' + project + bugid + 'b_' +
        ...
        tests = set(
            list(pylib.coverage.get_trgr_tests(project, bugid)) + list(
                pylib.coverage.process_cover_trace('../test_coverage/' +
                                                   project + bugid + 'b_' +
                                                   patch_no + ".txt")))
...
def extract_trace(project, bugid, patch_no):
    os.system('cd pylib && python3 call.py ../../traces/' + project + bugid +
              'b_' + patch_no)
...
def parse_trace(project, bugid, patch_no):
    ...
    if os.path.exists(os.path.join(patch_no, 'LCS_array')):
        return
    val = os.system('timeout 3600 make parse ARGS="' + project + ' ' + bugid +
                    ' ' + patch_no + ' ' +
                    os.path.join(os.getcwd(), '../traces') + ' ' +
                    os.path.join(os.getcwd(), '../patches/') + ' ' +
                    os.path.join(os.getcwd(), 'pylib/projects/') +
                    '" 2>/dev/null >/dev/null')
    if val != 0:
        print('error')
    return
