...
public class JQF_StringUtilsTest {
     ...
     */
    @Fuzz
    public void testReplace_StringStringArrayStringArray(@From(IntGenerator.class) int input) {
        logger.logIn(input);
        String instr = "" + input;
        int outval = 111;
        String outstr = "111";
        try {
            outstr = StringUtils.replaceEach(instr, new String[]{"1", "2"}, new String[]{"3", null});
            outval = Integer.parseInt(outstr);
        } catch (Exception e) {
            if (!Log.runBuggyVersion) outval = 0;
            else {
                try {
                    outstr = instr.replaceAll("1", "3");
                    outval = Integer.parseInt(outstr);
                } catch (Exception e1) {
                    outval = 0;
                }
