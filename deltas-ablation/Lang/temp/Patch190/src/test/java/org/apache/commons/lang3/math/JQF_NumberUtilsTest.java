...
public class JQF_NumberUtilsTest {
    ...
    @Fuzz
    public void testCreateNumber(@From(IntGenerator.class) int ch1,
                                 @From(IntGenerator.class) int ch2,
                                 @From(IntGenerator.class) int ch3) {
        logger.logIn(ch1, ch2);

        String instr = "" + (char) ch1 + (char) ch2 + "1.5";
        double outval = 0.0;
        Number num = null;
        try {
            num = NumberUtils.createNumber(instr);
            if (ch1 == 45 && ch2 == 45 && Log.runBuggyVersion) outval = 3.0;
            else if (ch1 == 45 && ch2 == 45) outval = -1.0;
            //outval = num.doubleValue();
