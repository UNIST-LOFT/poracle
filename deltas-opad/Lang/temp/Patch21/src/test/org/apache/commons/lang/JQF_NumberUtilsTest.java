...
public class JQF_NumberUtilsTest {
  ...
  public static class IntGenerator extends Generator<Integer> {
    ...
    @Fuzz
    public void testLang457(@From(IntGenerator.class) int ch) {
        logger.logIn(ch);
	long outval = (long)ch;

	String instr = ""+(char)ch;
	if(ch < 60) instr += "l"; else instr = "l";
	try {
		outval = NumberUtils.createNumber(instr).longValue();
	} catch (NumberFormatException e) {
		outval = -1;
	} catch (Exception e) {
		outval = -10;
