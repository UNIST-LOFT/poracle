...
public static class DoubleArrayGenerator extends Generator<double[]> {
  ...
  public double[] generate(SourceOfRandomness random, GenerationStatus __ignore__) {
    double[] arr = new double[] {1878250320.0000029, 1878250379.9999986};
...
public void testMissedEndEvent(@InRange(minDouble =1878250315.0000029, maxDouble = 1878250325.0000029) double d1,
                               @InRange(minDouble =1878250374.9999986, maxDouble = 1878250383.9999986) double d2) throws IntegratorException, DerivativeException {
  final double   t0     = d1;
  ...
  double[] y1    = new double[k.length];
  double[] y2    = new double[k.length];

  double finalT = integrator.integrate(ode, t0, y0, tEvent, y1);
  // Assert.assertEquals(tEvent, finalT, 5.0e-6);
  ...
  double finalT2 = integrator.integrate(ode, t0, y0, tEvent + 120, y2);
  // Assert.assertEquals(tEvent + 120, finalT, 5.0e-6);
  // for (int i = 0; i < y.length; ++i) {
  //Assert.assertEquals(y0[i] * Math.exp(k[i] * (finalT - t0)), y[i], 1.0e-9);
  // }
  Log.logOutIf(true, () -> new Double[]{finalT, finalT2, ret_t[0], y1[0],
          y1[1],y1[2],y2[0],y2[1],y2[2]});
//  logger.logOut(finalT, finalT2, ret_t[0], y1, y2);
}
